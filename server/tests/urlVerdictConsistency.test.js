...
it('A1 - credential URL + TI not_found -> classification matches heuristic output', () => {
  ...
  assert.ok(result.classification === 'legitimate' || result.classification === 'suspicious',
    `Expected legitimate or suspicious, got "${result.classification}" (riskLevel: ${result.riskLevel})`);
});
...
it('B1 - credential URL + TI unavailable -> classification matches heuristic output', () => {
  ...
  assert.ok(result.classification === 'legitimate' || result.classification === 'suspicious',
    `Expected legitimate or suspicious, got "${result.classification}"`);
});
...
it('C1 - credential URL + TI not_configured -> classification matches heuristic output', () => {
...
it('D3 - confirmed malicious TI -> riskScore >= 60 (high tier)', () => {
  const h = heuristicFromUrl('https://secure-login.account-verification.test/verify');
  const result = fuseEvidence(h, ML_NOT_APPLICABLE, tiMalicious(85), null);
